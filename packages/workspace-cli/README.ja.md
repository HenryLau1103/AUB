...
`--no-open` はブラウザを開かず、token を隠した editor URL だけを表示します。
手動で接続できる URL をコピーする必要がある場合は、信頼できるローカル端末で
`--no-open --print-auth-url` を使ってください。この URL にはローカル RPC token
が含まれるため、issue、PR、log、チャットには貼らないでください。
