...
`init` は AUB config、GitHub issue templates、Copilot instructions、PR workflow を作成します。app source は変更しません。生成対象が既に存在する場合は、`--force` を付けない限り上書きしません。
...
実装後は、実際の route に対して local evidence を capture できます。
...
これにより viewport screenshots、DOM checks、overflow checks、acceptance evidence が implementation report に入り、PR gate が Agent の自己申告だけに依存しなくなります。
...
Full workspace loop は local project files を読み書きするため、local editor と local MCP server が必要です。`npx aub-workspace init` で設定を作成し、`npx aub-workspace` でそれらをまとめて起動します。
