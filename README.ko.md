...
[Workspace loop 가이드](./docs/workspace-loop-user-manual.ko.md) · [GitHub agent workflow](./docs/github-agent-workflow.md) · [표준 예제](./examples/dashboard.ui.json)
...
```mermaid
flowchart LR
  A["1. 기존 app에서<br/>npx aub-workspace 실행"] --> B["2. route를 스캔하고<br/>candidate template 생성"]
  B --> C["3. mapping을 검토하고<br/>Blueprint 저장"]
  C --> D["4. 지시를 Copilot, Codex<br/>또는 다른 Agent에 전달"]
  D --> E["5. 구현 증거로<br/>PR 검증"]
```

1. **내 app에서 시작**: 기존 프로젝트 root에서 `npx aub-workspace`를 실행합니다. AUB clone은 필요 없습니다.
...
Editor 에서는 **Scan project → Generate template → Review component candidates → Save Blueprint/session → Copy agent instruction** 순서로 진행합니다. 그 지시를 Copilot, Codex 또는 다른 coding agent 에 전달해 실제 app 변경과 증거 보고를 요청합니다.
