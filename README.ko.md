...
**UI 계약을 정의하고, 프로덕션 컴포넌트를 재사용하며, 증거로 구현을 검증합니다.**
...
[Agent 전달 가이드](./docs/agent-handoff.md) · [표준 예제](./examples/dashboard.ui.json)

![반응형 화면을 편집하는 AUB 비주얼 에디터](./docs/assets/aub-editor-en.jpg)

AUB는 제품 의도와 코딩 Agent 사이의 개방형 agent-neutral 계약 계층입니다. 시맨틱 단일 화면 또는 다중 화면 프로젝트를 만들고, 사용자 정의 유형을 프로덕션 컴포넌트에 연결한 뒤, 동일한 버전 관리 계약을 Codex, Claude Code, GitHub Copilot 또는 다른 Agent에 전달하고 CI에서 모든 acceptance id를 검증합니다.
...
```mermaid
flowchart LR
  A["1. 비주얼 아트보드에서<br/>UI 구성"] --> B["2. 구조, 동작, 반응형 규칙,<br/>인수 조건 내보내기"]
  B --> C["3. 패키지를<br/>코딩 Agent에 전달"]
  C --> D["4. Agent가 구현하고 테스트한 뒤<br/>증거 보고"]
```

1. **구성 또는 가져오기**: 18개 템플릿, 등록된 컴포넌트, Angular, Figma／Penpot Design Bridge에서 시작합니다.
2. **프로덕션 컴포넌트 연결**: 사용자 정의 시맨틱 유형을 framework module, export, source, Storybook, 문서, props에 연결합니다.
