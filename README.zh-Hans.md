...
[Workspace loop 指南](./docs/workspace-loop-user-manual.zh-Hans.md) · [GitHub agent workflow](./docs/github-agent-workflow.md) · [标准示例](./examples/dashboard.ui.json)
...
```mermaid
flowchart LR
  A["1. 在既有 app 执行<br/>npx aub-workspace"] --> B["2. 扫描 route 并生成<br/>candidate template"]
  B --> C["3. 审核 mapping、调整并<br/>保存 Blueprint"]
  C --> D["4. 把指令交给<br/>Copilot、Codex 或其他 Agent"]
  D --> E["5. 用实现证据<br/>管控 PR"]
```

1. **从你的 app 启动**：在既有项目根目录执行 `npx aub-workspace`，不需要 clone AUB。
...
进入 editor 后按这条路径走：**扫描项目 → 生成模板 → 审核自定义组件候选 → 保存 Blueprint/session → 复制 Agent 指令**。把指令贴给 Copilot、Codex 或其他 coding agent，让它修改真实 app 并返回证据。
